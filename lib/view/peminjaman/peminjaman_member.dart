import 'package:belajar_flutter_perpus/view/peminjaman/detail_peminjaman_member.dart';
import 'package:flutter/material.dart';
import 'package:dio/dio.dart';
import 'dart:developer';

class PeminjamanMemberPage extends StatefulWidget {
  final String token;
  final int memberId;

  PeminjamanMemberPage({required this.token, required this.memberId});

  @override
  _PeminjamanMemberPageState createState() => _PeminjamanMemberPageState();
}

class _PeminjamanMemberPageState extends State<PeminjamanMemberPage> {
  ...
  Future<void> fetchPeminjaman() async {
    try {
      final response = await Dio().get(
        'http://perpus-api.mamorasoft.com/api/peminjaman/all',
        options: Options(
          headers: {
      ...
      if (data != null && data['data'] != null) {
        // Filter peminjaman berdasarkan memberId dan status
        final filteredPeminjaman =
            data['data']['peminjaman'].where((peminjaman) {
          final memberId = peminjaman['member']['id'] ?? 0;
          final status = peminjaman['status'] ?? '0';
          return memberId == widget.memberId &&
              (status == '1' || status == '2');
        }).toList();
